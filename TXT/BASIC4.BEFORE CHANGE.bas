' PROGRAM DXF
DIM DXFINPUTRAW(4000) AS STRING
...
DIM COUNTINGROUP(10) AS INTEGER
C = 0
F = 0
A = 0
DIMSCT = 0
...
FOR POTT = 1 TO 4
    FOR POT = 1 TO COUNTINGROUP(POTT)
        ...
        PRINT #3, "N"; LTRIM$(STR$(LINESZ));
        IF X1FINAL(POT, POTT) <> X1FINAL(POT - 1, POTT) AND Y1FINAL(POT, POTT) <> Y1FINAL(POT - 1, POTT) THEN
            PRINT #3, USING TEMPL1$; LTRIM$(STR$(CSNG(X1FINAL(POT, POTT)))); LTRIM$(STR$(CSNG(Y1FINAL(POT, POTT))))
        END IF
        IF X1FINAL(POT, POTT) = X1FINAL(POT - 1, POTT) AND Y1FINAL(POT, POTT) <> Y1FINAL(POT - 1, POTT) THEN
            PRINT #3, USING TEMPL2$; LTRIM$(STR$(CSNG(Y1FINAL(POT, POTT))))
        END IF
        IF X1FINAL(POT, POTT) <> X1FINAL(POT - 1, POTT) AND Y1FINAL(POT, POTT) = Y1FINAL(POT - 1, POTT) THEN
            PRINT #3, USING TEMPL3$; LTRIM$(STR$(CSNG(X1FINAL(POT, POTT))))
        END IF


        LINESZ = LINESZ + 1
        PRINT #3, "N" + LTRIM$(STR$(LINESZ)) + "Q23"
        '  SLEEP
    NEXT POT
NEXT POTT
